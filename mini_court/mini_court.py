...
class MiniCourt:
    ...
    def draw_court(self, frame):
        ...
        for line in self.lines:
            pt1 = (
                int(self.keypoints[line[0] * 2]),
                int(self.keypoints[line[0] * 2 + 1])
            )
            pt2 = (
                int(self.keypoints[line[1] * 2]),
        ...
        y_top = int(self.keypoints[1])
        y_bottom = int(self.keypoints[5])

        cv2.line(frame, (left_x, y_top), (left_x, y_bottom), (0, 0, 0), 2)
        cv2.line(frame, (right_x, y_top), (right_x, y_bottom), (0, 0, 0), 2)

        service_top_y = int(self.keypoints[9] + self.convert_meters_to_pixels(constants.NO_MANS_LAND_HEIGHT))
        service_bottom_y = int(self.keypoints[11] - self.convert_meters_to_pixels(constants.NO_MANS_LAND_HEIGHT))
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self, ball_boxes, original_court_keypoints):
        ...
        # Extract full real court bounds
        ...
        for ball_box_dict in ball_boxes:
            ...
            norm_x = (ball_x - real_x1) / real_width
            norm_y = (ball_y - real_y1) / real_height
